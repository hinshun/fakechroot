# pod2man --section=1 --release="fakechroot" --center=" " --date="16 Mar 2019"
#
...
C<LD_LIBRARY_PATH> variable. The directories might be used by some important
commands which use libraries placed in the I<runpath>. In that case dynamic
linker can't find correct directory in a fake chroot environment without the
extra library path.

The default value is C</lib/systemd:/usr/lib/man-db> for systemctl(1) and
man(1) commands.
